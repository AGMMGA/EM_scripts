...
class test_args_check(unittest.TestCase):
    ...
    def setUp(self):
        sys.argv = shlex.split('test.py -i /tmp/in.star -o /tmp/out.star -k 1 2 3 -filename=123_BRCA1_001.mrc -digits 4')
...
class test_run_as_module(unittest.TestCase):
    ...
    def test_no_lst(self):
        with self.assertRaises(ValueError):
            s.run_as_module(self.starfile_in, self.starfile_out,
                self.filename, self.digits, [], mode='a', force=True)
            
    def test_class_setup(self):
        _, self.argv = s.run_as_module(self.starfile_in, self.starfile_out,
                self.filename, self.digits, [1,2,3], mode='a', force=True)
        ...
        self.assertTrue(hasattr(self.argv, 'a'))
        self.assertEqual(self.argv.a, [1,2,3])
...
class test_write_file(unittest.TestCase):
    ...
    def setUp(self):
        self.starfile_in = '/tmp/star1.star'
        self.ctf_file_in = '/tmp/star2.star'
        self.starfile_out = '/tmp/out.star'
        self.filename = '123_BRCA1_001.mrc'
        self.digits = 3
        with open('/tmp/star1.star', 'w') as f:
...
'''
    ...
    def test_add_mode_normal_star(self):
        self.maxDiff = None
        add = ['001','002']
        s.write_file(self.starfile_in, self.starfile_out, 
                     self.filename, self.digits, add, mode='a')
        with open(self.starfile_out, 'r') as f:
            self.assertMultiLineEqual(f.read(), self.normal_starfile_out)
...
class test_all_exist(unittest.TestCase):
